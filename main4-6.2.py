...
# --- 6. 범주형 변수 더미 코딩 (One-Hot Encoding) ---
...
models = {
    ...
    'LightGBM': LGBMRegressor(
        random_state=42,
        min_child_samples=5,  # 데이터가 적은 범주형(더미) 변수도 무시하지 않도록 조건 완화
        max_depth=5,
        learning_rate=0.05,
        n_estimators=200,
        verbose=-1
...
# --- 8. 특성 중요도(Feature Importance) 일관성 비교 시각화 ---
...
plt.tight_layout()
plt.show(block=False)  # 다음 출력을 위해 창이 닫히지 않아도 코드 진행

# --- 9. 최우수 모델 변수 중요도 구체적 수치 출력 ---
print(f"\n=== 🏆 {best_model_name} 변수 중요도 (Feature Importance %) ===")
...
# --- 10. SHAP Value 기반 최종 해석 (Best Model 사용) ---
print(f"최고 성능 모델({best_model_name})을 바탕으로 SHAP Value 검증을 시작합니다...")
...
print("\n🎉 머신러닝 모델 학습, 수치 출력, 그리고 SHAP 분석이 완벽하게 종료되었습니다!")