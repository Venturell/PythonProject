...
GOOGLE_API_KEY = "여기에_GOOGLE_API_입력"
...
output_path = os.path.join(os.path.dirname(file_path), "10_PAproject_10_3_Map_Google.html")

print("데이터를 불러오고 Google Maps API를 통해 주소를 좌표로 변환합니다...\n")
...
def get_lat_lon_google(address):
    """Google Maps API를 사용하여 주소를 위도/경도로 변환"""
    if pd.isna(address):
        return None, None

    try:
        geocode_result = gmaps.geocode(str(address), language='ko')
        if geocode_result:  # 변환 결과가 존재하는 경우
